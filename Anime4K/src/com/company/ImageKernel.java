...
// final 作用类似inline ，调用此类函数时直接展开，但是final修饰的类不可被继承， 修饰的函数不能被重写，修饰的变量必须初始化值且不能改变
// extends 表示继承
// 一般语法为 ：class ChildClass extends ParentClass { ... }
    ...
    private byte[] derivData;   //值域被定义为-128~127
    private int[] tempColorData;
    private byte[] tempDerivData;

    private boolean isForwardPass = true;  // 是否前向传播
    ...
    public void setBufferedImage(BufferedImage img) {  // 获得缓冲区图像数据

        if (width != img.getWidth() || height != img.getHeight()) {
            colorData = ((DataBufferInt)img.getRaster().getDataBuffer()).getData();  // 获得图像像素数组
